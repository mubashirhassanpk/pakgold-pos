...
import { addLedgerEntry, deleteLedgerEntry } from "@/lib/karigarActions";
import { EARNING_KINDS } from "@/lib/constants";
...
const KINDS = ["salary", "dehari", "commission", "bonus", "payout", "advance", "deduction"];
const isEarning = (k: string) => (EARNING_KINDS as readonly string[]).includes(k);
...
export function LedgerSection({
  ...
  const [kind, setKind] = useState("dehari");
  ...
  function add(k = kind, amt = amount, nt = note) {
    setError("");
    start(async () => {
      const res = await addLedgerEntry(karigarId, { kind: k, amount: amt, note: nt, entryDate: date || undefined });
  ...
  function payFull() {
    if (balance > 0) add("payout", balance, "Full settlement");
  }

  function remove(id: number) {
    if (!confirm("Delete this entry?")) return;
    start(async () => {
      await deleteLedgerEntry(id, karigarId);
      router.refresh();
    });
  }
  ...
  return (
    <div className="space-y-5">
      ...
      <div className={`rounded-2xl p-5 ring-1 ring-black/5 flex items-center justify-between ${balance > 0 ? "bg-red-50" : "bg-white"}`}>
        <div>
          <div className="text-xs text-gray-500">Balance Payable</div>
        ...
        {balance > 0 && (
          <button onClick={payFull} disabled={pending} className="rounded-lg bg-success text-white text-sm font-semibold px-4 py-2 hover:brightness-105 disabled:opacity-60">
            Pay Full
          </button>
        )}
      </div>
      ...
      <div className="rounded-2xl bg-white ring-1 ring-black/5 p-4">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-5 gap-2">
          ...
          <input value={note} onChange={(e) => setNote(e.target.value)} placeholder="Note" className="col-span-2 rounded-lg border border-gray-200 px-2 py-2 text-sm" />
        ...
        <button onClick={() => add()} disabled={pending || amount <= 0} className="mt-3 rounded-lg bg-navy-900 text-white text-sm font-semibold px-4 py-2 hover:bg-navy-800 disabled:opacity-50">
          {pending ? "Saving…" : "Add"}
        </button>
        <p className="text-xs text-gray-400 mt-2">
          Earnings (salary, dehari, commission, bonus) increase what you owe. Payout/advance/deduction reduce it.
        </p>
      </div>
          ...
          <tbody className="divide-y divide-gray-100">
            {entries.map((e) => (
              <tr key={e.id}>
                ...
                <td className="px-4 py-2"><span className={`rounded-full px-2 py-0.5 text-xs font-medium capitalize ${isEarning(e.kind) ? "bg-gold-100 text-gold-700" : "bg-gray-100 text-gray-600"}`}>{e.kind}</span></td>
                <td className="px-4 py-2 text-gray-600">{e.note || "—"}</td>
                <td className={`px-4 py-2 text-right tnum font-semibold ${isEarning(e.kind) ? "text-success" : "text-red-600"}`}>
                  {isEarning(e.kind) ? "+" : "−"} {formatPKR(e.amount)}
                </td>
